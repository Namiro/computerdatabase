...
public class CompanyServiceTest {
     ...
     *
     * - The user tries to enter a company a long name. tested with
     * testCheckDataEntityNameSoLong - The user tries to enter a company with
     * no name tested with testCheckDataEntityNoName - The user tries to enter
     * a company with a bad id tested with testCheckDataBadId - The user tries
     * to enter a company with no entity tested with testCheckDataNoEntity
     */
    @Test
    public void testCheckDataEntityNameSoLong() {
        final Company company = new Company.CompanyBuilder()
                ...
                .build();
        this.exception.expect(TooLongCompanyNameException.class);
        this.companyService.checkDataEntity(company);
    }
    ...
    public void testCheckDataEntityNoName() throws ServiceException {
        ...
        this.exception.expect(TooShortCompanyNameException.class);
        this.companyService.checkDataEntity(company);
    }
    ...
    /*
     * Following tests are here to check wether we can get companies according
     * to the client needs. Here we shall test if:
     *
     * - The user tries to get several companies tested with testGet - The
     * user tries to get a single company tested with testGetSingleCompany -
     * The user tries to get a company page tested with testGetPage - The user
     * tries to get a company with a bad id tested with testGetWithBadId - The
     * user tries to get a company by id tested with testGetWithId
     */
    @Test
    public void testGet() {
    ...
    public void testGetPage() {
        ...
        Assert.assertEquals(this.companyService.getPage(Company.class, 1, 3, "CM", OrderCompanyEnum.NAME).size(),
                3);
        Assert.assertEquals(
                this.companyService.getPage(Company.class, 1, 3, "CM", OrderCompanyEnum.NAME).get(0).getName(),
                "AAA");
        Assert.assertEquals(
                this.companyService.getPage(Company.class, 1, 3, "CM", OrderCompanyEnum.NAME).get(1).getName(),
                "BBB");
        Assert.assertEquals(
                this.companyService.getPage(Company.class, 1, 3, "CM", OrderCompanyEnum.NAME).get(2).getName(),
                "CCC");
    }

    @Test
    public void testGetSingleCompany() {
        final Optional<Company> company = Optional.of(new Company.CompanyBuilder().id(34).name("test").build());
        ReflectionTestUtils.setField(this.companyService, "dao", this.mockCompanyDao);
        Mockito.when(this.mockCompanyDao.findById(Company.class, 34)).thenReturn(company);
        Assert.assertEquals(this.companyService.get(Company.class, 34).get().getId(), 34);
        Assert.assertEquals(this.companyService.get(Company.class, 34).get().getName(), "test");
    }
     ...
     *
     * - The user tries to remove a company tested with testRemove - The user
     * tries to remove a company with a bad Id tested with testRemoveWithBadId
     */
    @Test
    public void testRemove() {
        ...
        Assert.assertTrue(this.companyService.remove(company));
    }
    ...
    public void testRemoveWithBadId() {
        ...
        Assert.assertFalse(this.companyService.remove(company));
    }
